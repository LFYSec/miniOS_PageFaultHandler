...
PRIVATE void mkfs()
{
	...
	RD_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + cur_sect, fsbuf);	//modified by mingxuan 2019-5-19
	while (bit_left) {
		...
		if (bit_off_in_sect == (SECTOR_SIZE * 8)) {
			...
			WR_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + cur_sect, fsbuf);	//modified by mingxuan 2019-5-19
			cur_sect++;
			//RD_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + cur_sect);	//deleted by mingxuan 2019-5-19
			RD_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + cur_sect, fsbuf);	//modified by mingxuan 2019-5-19
			bit_off_in_sect = 0;
