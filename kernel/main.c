...
PUBLIC int kernel_main()
{
	...
	************************************************************************/
	...
	init_vfs();		//added by mingxuan 2019-5-17
...
PRIVATE int initialize_processes()
{
	...
	***************************************************************************/
	...
	{//1>对前NR_TASKS个PCB初始化,且状态为READY(生成的进程)
		...
		link_init(&p_proc->task.pages);
		link_init(&p_proc->task.memmap.text_pages);
		link_init(&p_proc->task.memmap.data_pages);
		link_init(&p_proc->task.memmap.heap_pages);
		link_init(&p_proc->task.memmap.stack_pages);
        link_init(&p_proc->task.memmap.arg_pages);
		// link_init(&p_proc->task.memmap.kernel_pages);
		// link_init(&p_proc->task.memmap.vpage_pages);
		...
		for(AddrLin=StackLinBase; AddrLin>p_proc->task.memmap.stack_lin_limit; AddrLin-=num_4K){
			...
			link_append(&p_proc->task.pages, &(page->node));
			link_append(&p_proc->task.memmap.stack_pages, &(page->node_area));
		}
	...
	{//initial 进程的初始化				//add by visual 2016.5.17
		...
		/*for(AddrLin=StackLinBase; AddrLin>p_proc->task.memmap.stack_lin_limit; AddrLin-=num_4K){*/
			...
			/*page->v_addr = ((AddrLin>>12)<<12);*/
			/*page->flag = PG_ALLOCATED;*/

			/*link_append(&p_proc->task.pages, &(page->node));*/
            
            /*// TODO remove. Testing swapout*/
            /*//swapout(page, p_proc->task.cr3);*/
		/*}*/
